% Machine Vision HW4 --- Tracking
clear all; close all; clc

%% Read video into MATLAB using aviread
video = VideoReader('C:\Users\Evarist\OneDrive - Florida State University\Headway_1\clip111.avi');
nframes = round(video.Duration*video.FrameRate);

% Calculate the background image by averaging the first 5 images
temp = zeros(size(video(1)));
[M,N] = size(temp(:,:,1));
for i = 1:10   
    temp =read(video, i) + temp;
end
imbkg = temp/10;
...
for i=1:nframes
  imshow(read(video, i));
  hold on
  imcurrent = double(read(video, i));
  
%   % Calculate the background based on n=10 former frames
%   % If i<=10, use the initialized background
%   temp = zeros(size(video(1).cdata));
%   if i <= 10
%       imbkg1 = imbkg;
%   else
%       for ii = 1:10
%       temp = (11-ii).*double(video(i-ii).cdata) + temp;
%       end
%       imbkg1 = temp./10;
%   end
  ...
  diffimg = zeros(M,N);  
  diffimg = (abs(imcurrent(:,:,1)-double(imbkg(:,:,1)))>th) ... 
      | (abs(imcurrent(:,:,2)-double(imbkg(:,:,2)))>th) ... 
      | (abs(imcurrent(:,:,3)-double(imbkg(:,:,3)))>th);
  ...
  % Apply Kalman filter to get the predicted and actual centroid and volocity
  % Kalman update --- time update(prediction) and measurement update(correction)
  if kfinit == 0
